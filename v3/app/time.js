...
function studyDeadlineItems(state,date){return list(state?.education?.items).filter(item=>!item.done&&text(item.dueDate)===date).map(item=>timeItem({source:'study-deadline',sourceId:item.id,id:`study-deadline:${item.id}`,title:item.title||'Study deadline',type:'deadline',date,startTime:item.dueTime,allDay:!item.dueTime,protected:item.protected===true||item.type==='exam',detail:`Study Nook · ${item.type||'item'}`}))}
function threadDeadlineItems(state,date){return list(state?.life?.threads).filter(item=>item.status!=='complete'&&item.status!=='archived'&&text(item.deadline)===date).map(item=>timeItem({source:'thread-deadline',sourceId:item.id,id:`thread-deadline:${item.id}`,title:item.title||'Thread deadline',type:'deadline',date,allDay:true,protected:false,detail:'Thread target date'}))}
function goalTargetItems(state,date){return list(state?.growth?.goals).filter(item=>item.status!=='complete'&&text(item.targetDate)===date).map(item=>timeItem({source:'goal-target',sourceId:item.id,id:`goal-target:${item.id}`,title:item.title||'Goal target',type:'deadline',date,allDay:true,protected:false,detail:'Growth target date'}))}
