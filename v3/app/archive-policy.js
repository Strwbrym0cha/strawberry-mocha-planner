export const ARCHIVE_POLICY_VERSION=2;
...
function nativeArchived(state,kind,id){const target=text(id);if(kind==='thread')return list(state.life?.threads).some(x=>String(x.id)===target&&x.status==='archived');if(kind==='routine')return list(state.life?.routines).some(x=>String(x.id)===target&&x.archived===true);return false}
export function isArchived(state,kind,id){const k=text(kind),target=text(id);return nativeArchived(state,k,target)||archiveRefs(state).some(x=>x.kind===k&&x.id===target)}
