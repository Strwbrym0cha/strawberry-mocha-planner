...
export const CONTEXT_OPTIONS={
  ...
  mode:[
    ...
    {value:'boss',icon:'💼',label:'Boss Bitch'},
...
export const DEFAULT_CONTEXT={brain:'steady',energy:'okay',capacity:'normal',pressure:'chill',socialBattery:'neutral',mode:'normal',currentActivity:'',note:'',updatedAt:''};
const allowed=(key,value)=>CONTEXT_OPTIONS[key]?.some(option=>option.value===value);

export function normalizeContext(value){
  const saved=value&&typeof value==='object'&&!Array.isArray(value)?value:{},next={...DEFAULT_CONTEXT,...saved};
  for(const key of ['brain','energy','capacity','pressure','socialBattery','mode'])if(!allowed(key,next[key]))next[key]=DEFAULT_CONTEXT[key];
  next.currentActivity=String(next.currentActivity||'').trim();next.note=String(next.note||'').trim();next.updatedAt=String(next.updatedAt||'');return next;
}
export function updateContext(context,patch={}){return normalizeContext({...normalizeContext(context),...patch,updatedAt:new Date().toISOString()})}
export function contextLabel(key,value){return CONTEXT_OPTIONS[key]?.find(option=>option.value===value)||{value,icon:'',label:String(value||'')}}

export function inferContextPatch(text){
  const input=String(text||'').toLowerCase(),patch={};
  ...
  if(/work mode|boss mode|boss bitch|i(?:'m| am) working|time to work|clock(?:ing)? in/.test(input))patch.mode='boss';
  ...
  if(/normal mode|back to normal|regular mode|clock(?:ing)? out|done working/.test(input))patch.mode='normal';
  return patch;
}
export function describeContextPatch(patch={}){return Object.entries(patch).filter(([key,value])=>CONTEXT_OPTIONS[key]?.some(option=>option.value===value)).map(([key,value])=>({key,...contextLabel(key,value)}))}
