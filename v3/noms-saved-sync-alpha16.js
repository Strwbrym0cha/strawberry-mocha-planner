import{loadV3State,saveV3State}from'./app/schema.js?v=3.0.0-alpha.16.3-stockguard';
import{normalizeNoms,logNom}from'./app/noms.js?v=3.0.0-alpha.16-stockguard';
...
function saveLiveNoms(noms){const state=loadV3State(),next=normalizeNoms(noms),saved=saveV3State({...state,nourish:{...state.nourish,noms:next}});window.__katOSV3=saved;window.dispatchEvent(new CustomEvent('katos:noms-stock-changed',{detail:{noms:next}}));return next}
...
document.addEventListener('click',event=>{
  ...
  paintSavedFoods(next);paintFridge(next);removeOutRecommendation(id,fresh);flash(button,`✓ Logged · ${fresh?.quantity??0} left`);
},true);
