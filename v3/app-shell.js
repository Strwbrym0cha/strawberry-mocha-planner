const BUILD='3.0.0-alpha.16-sidebar1';
...
const hashView=()=>{const h=location.hash.replace('#','').toLowerCase();if(['tasks','sweet-todos'].includes(h))return'tasks';if(['mochini'].includes(h))return'mochini';if(['sips','sip-station'].includes(h))return'sips';if(['pings','little-pings'].includes(h))return'pings';return'home'};
function activeId(){if(isHomePath())return hashView();const file=filename();const byFile={
  'time.html':'time','routines.html':'routines','noms.html':'noms','motion.html':'motion','boss.html':'boss','money.html':'money','study.html':'study','threads.html':'threads','growth.html':'growth','control.html':'control','archive.html':'archive','cloud.html':'cloud','migrate.html':'migrate','launch.html':'launch'
};if(file==='mind.html')return location.hash==='#patterns'?'patterns':'reset';return byFile[file]||'home'}
...
function labelCoreViews(){if(!isHomePath())return;const sections=[...document.querySelectorAll('#app section')];for(const section of sections){let view='';if(section.classList.contains('home'))view='home';const heading=(section.querySelector('h2')?.textContent||'').trim().toLowerCase();if(heading==='current context')view='home';else if(heading==='sweet to-dos')view='tasks';else if(heading==='sip station')view='sips';else if(heading==='mochini')view='mochini';else if(heading==='little pings')view='pings';if(view)section.dataset.katosCoreView=view}
  const selected=hashView();document.querySelectorAll('[data-katos-core-view]').forEach(section=>section.classList.toggle('katos-view-hidden',section.dataset.katosCoreView!==selected));
  document.body.dataset.katosPage='home';
}
