const BUILD='3.0.0-alpha.16-sidebar5-dayreview';
const NAV_SCROLL_KEY='sm_v3_sidebar_scroll',PAGE_SCROLL_KEY='sm_v3_page_scroll';
...
const NAV=[
  {label:'🏡 HOME',items:[
    ...
    {id:'review',icon:'🌙',label:'Day in Review',href:page('review.html')},
...
function saveSidebarScroll(){const nav=document.getElementById('katosSidebarNav');if(nav)try{sessionStorage.setItem(NAV_SCROLL_KEY,String(nav.scrollTop))}catch{}}
function restoreSidebarScroll(){const nav=document.getElementById('katosSidebarNav');if(!nav)return;let saved=0;try{saved=Number(sessionStorage.getItem(NAV_SCROLL_KEY)||0)}catch{}requestAnimationFrame(()=>{nav.scrollTop=saved})}
function savePageScroll(){try{sessionStorage.setItem(PAGE_SCROLL_KEY,String(window.scrollY||0))}catch{}}
function restorePageScroll(){let saved=0;try{saved=Number(sessionStorage.getItem(PAGE_SCROLL_KEY)||0)}catch{}requestAnimationFrame(()=>window.scrollTo({top:saved,left:0,behavior:'auto'}))}
function bindShell(){const nav=document.getElementById('katosSidebarNav');document.getElementById('katosMenuButton')?.addEventListener('click',()=>openDrawer(!document.body.classList.contains('katos-drawer-open')));document.getElementById('katosDrawerScrim')?.addEventListener('click',()=>openDrawer(false));nav?.addEventListener('scroll',saveSidebarScroll,{passive:true});document.querySelectorAll('[data-katos-nav]').forEach(a=>a.addEventListener('click',()=>{saveSidebarScroll();savePageScroll();openDrawer(false)}));document.addEventListener('keydown',event=>{if(event.key==='Escape')openDrawer(false)});window.addEventListener('pagehide',()=>{saveSidebarScroll();savePageScroll()})}
...
function install(){if(document.getElementById('katosSidebar'))return;document.body.classList.add('katos-v3-shell');if(!isHomePath())document.body.dataset.katosPage=activeId();document.body.insertAdjacentHTML('afterbegin',shellMarkup());bindShell();restoreSidebarScroll();applyViews();const app=document.getElementById('app');if(app){let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;refreshChrome()})}).observe(app,{childList:true,subtree:true})}window.addEventListener('hashchange',()=>{const before=Number(sessionStorage.getItem(PAGE_SCROLL_KEY)||window.scrollY||0);refreshChrome();requestAnimationFrame(()=>window.scrollTo({top:before,left:0,behavior:'auto'}))});window.addEventListener('storage',event=>{if(event.key==='sm_v3_beta')refreshChrome()});refreshChrome();import('./archive-enhancer-alpha16.js?v=3.0.0-archive1').catch(()=>{});import('./day-review-launcher-alpha16.js?v=3.0.0-dayreview1').catch(()=>{})}
