const BUILD='3.0.0-alpha.16-sidebar3';
...
function install(){if(document.getElementById('katosSidebar'))return;document.body.classList.add('katos-v3-shell');if(!isHomePath())document.body.dataset.katosPage=activeId();document.body.insertAdjacentHTML('afterbegin',shellMarkup());bindShell();restoreSidebarScroll();applyViews();const app=document.getElementById('app');if(app){let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;refreshChrome()})}).observe(app,{childList:true,subtree:true})}window.addEventListener('hashchange',()=>{const before=Number(sessionStorage.getItem(PAGE_SCROLL_KEY)||window.scrollY||0);refreshChrome();requestAnimationFrame(()=>window.scrollTo({top:before,left:0,behavior:'auto'}))});window.addEventListener('storage',event=>{if(event.key==='sm_v3_beta')refreshChrome()});refreshChrome()}
