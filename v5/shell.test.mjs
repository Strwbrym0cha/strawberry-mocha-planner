...
for(const selector of ['.sidebar{','.topbar{','.mode-switch{','.detail-modal-backdrop{','body.mode-tiny','body.mode-power'])assert.equal(styles.includes(selector)||readFileSync(resolve(folder,'detailed-rooms.css'),'utf8').includes(selector),true,`${selector} should remain in the V5 shell`);
