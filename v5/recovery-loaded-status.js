...
function packedState(){const row=current();if(!row?.state)throw new Error('KatOS could not read the recovered local planner.');const state=structuredClone?structuredClone(row.state):JSON.parse(JSON.stringify(row.state));const aux={};for(const key of AUX_KEYS){const raw=localStorage.getItem(key);if(raw==null)continue;try{aux[key]=JSON.parse(raw)}catch{aux[key]=raw}}state.__katosAuxStores=aux;state.__recoveryAcceptedAt=new Date().toISOString();return state}
async function api(session,path,options={}){const response=await fetch(`${CLOUD_URL}/rest/v1/${path}`,{...options,headers:{apikey:CLOUD_KEY,Authorization:`Bearer ${session.access_token}`,...(options.headers||{})}});const payload=await response.json().catch(()=>null);if(!response.ok)throw new Error(payload?.message||payload?.hint||'KatOS could not save the recovered planner.');return payload}
async function keepRecoveredCopy(){const session=storedSession();if(!session)throw new Error('Sign in to KatOS on this iPad first.');const state=packedState();const rows=await api(session,`${TABLE}?user_id=eq.${encodeURIComponent(session.user.id)}&select=data,revision,updated_at,last_device_id`);const existing=Array.isArray(rows)?rows[0]:null;if(existing?.data){await api(session,SNAPSHOTS,{method:'POST',headers:{'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({user_id:session.user.id,data:existing.data,schema_version:1,revision:Number(existing.revision||1),reason:'before-accepted-recovery',device_id:'recovery-finalize'})});}
