...
function saveSession(session){if(!session)return;activeRecoverySession=session;for(const key of['sm_v16_session','sm_cloud_session'])try{localStorage.setItem(key,JSON.stringify(session))}catch{}}
...
async function validSession(){let session=activeRecoverySession||storedSession();if(!session)throw new Error('Sign in to KatOS on this iPad first.');const expires=tokenExpiry(session);if(expires&&expires-Date.now()<120000){session=await refreshSession(session);if(!session)throw new Error('Your KatOS cloud sign-in expired. Your recovered planner is still safe on this iPad. Sign in again, then tap Keep this recovered copy.');}activeRecoverySession=session;return session}
...
function rememberPromoted(revision){runtimePromoted=String(revision);try{sessionStorage.setItem(PROMOTED_KEY,runtimePromoted)}catch{}try{localStorage.setItem(PROMOTED_KEY,runtimePromoted)}catch{}}
function knownPromoted(){if(runtimePromoted)return runtimePromoted;try{const s=sessionStorage.getItem(PROMOTED_KEY);if(s)return s}catch{}try{const l=localStorage.getItem(PROMOTED_KEY);if(l)return l}catch{}return''}
async function cloudAcceptedRevision(){try{const session=await validSession();const rows=await api(session,`${TABLE}?user_id=eq.${encodeURIComponent(session.user.id)}&select=revision,last_device_id`,{},true);const row=Array.isArray(rows)?rows[0]:null;return row?.last_device_id==='accepted-recovery'?Number(row.revision||0):0}catch{return 0}}
async function keepRecoveredCopy(){let session=await validSession();const state=packedState();const rows=await api(session,`${TABLE}?user_id=eq.${encodeURIComponent(session.user.id)}&select=data,revision,updated_at,last_device_id`);session=activeRecoverySession||session;const existing=Array.isArray(rows)?rows[0]:null;if(existing?.data){await api(session,SNAPSHOTS,{method:'POST',headers:{'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({user_id:session.user.id,data:existing.data,schema_version:1,revision:Number(existing.revision||1),reason:'before-accepted-recovery',device_id:'recovery-finalize'})});session=activeRecoverySession||session}
