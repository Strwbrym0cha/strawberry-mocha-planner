import{cloudSignedIn,cloudSignOut,syncNow}from'./cloud-sync.js?v=5.8.0-cross-device-sync';

const CLOUD_URL='https://sigjwmgekmrwehylvuvu.supabase.co';
const CLOUD_KEY='sb_publishable_MrtltNJt8EufDKM1UqqDCQ_JphJneJt';
const CLOUD_SYNC_SESSION='sm_cloud_session';
...
function storedSession(){for(const key of[CLOUD_SYNC_SESSION,SESSION_KEY,SUPABASE_SESSION_KEY]){try{const session=normalizeSession(JSON.parse(localStorage.getItem(key)||'null'));if(session)return session}catch{}}return null}
...
async function signIn(email,password){
 ...
 localStorage.setItem(CLOUD_SYNC_SESSION,JSON.stringify(payload));
