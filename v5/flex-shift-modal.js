import{archiveV5Record,localDateKey,runV5MoneyGigAction,saveV5GigShift,selectV5MoneyGig,snapshotV4,updateV5Record}from'./data.js?v=7.0.22-odometer-sunday-payout';
...
app.addEventListener('submit',event=>{
 ...
 const packageCount=Math.max(0,Number(data.packageCount)||0),actualMinutes=Math.max(0,(Number(data.actualHours)||0)*60+(Number(data.actualMinutesPart)||0)),orderResult=runV5MoneyGigAction({type:'order-save',id:form.dataset.summaryId||'',platformId:platformResult.result.id,date:text(data.date)||localDateKey(),basePay:actualPay,tip:0,bonus:0,promo:0,reimbursement:0,otherPay:0,startOdometer:data.startOdometer,endOdometer:data.endOdometer,activeMinutes:actualMinutes,onlineMinutes:actualMinutes,status:'completed',notes:text(data.summaryNotes)});
