...
export class SafeSyncEngine{
  ...
  async fetchCloudDiagnostics(){
    ...
    try{
      ...
      const readEndpoint=async(endpoint,extraHeaders={})=>{
        ...
        if(response.status!==401)return{response,authFailure:null};
        const refreshed=await refreshSession(session,{fetchFunction:this.fetchFunction,storage:this.storage});
        if(refreshed.state!=='AUTHENTICATED')return{response:null,authFailure:refreshed};
        session=refreshed.session;
        auth=publicAuthDiagnostics({...refreshed,sourceKey:SYNC_SESSION_KEY});
        response=await request();
        return{response,authFailure:null};
      };
      ...
      const plannerRead=await readEndpoint(endpoint);
      ...
      try{
        ...
        const snapshotRead=await readEndpoint(snapshotEndpoint,{Prefer:'count=exact'});
