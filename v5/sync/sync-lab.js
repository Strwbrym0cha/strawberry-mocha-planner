...
function recoveryPlanSummary(report){
  ...
  return`<div style="margin-top:14px"><div class="ey">RECOVERY PLAN PREVIEW · READ ONLY</div><p><b>Baseline: Local iPad</b> · ${report.decisions.length} non-identical/unmatched records classified · ${failed.length} integrity categories need review.</p><p><b>Recovery readiness:</b> ${escapeHtml(report.readiness)}</p><div class="chip-row">${actions}</div><div class="room-list" style="margin-top:10px"><div><b>Proposed recovered dataset</b><br>${escapeHtml(shortHash(report.preview.contentHash))} · ${escapeHtml(report.preview.serializedBytes)} bytes · ${report.countChanges.length?`${report.countChanges.length} explained count changes`:'all collection counts match local'}</div>${integrity}</div><div class="button-row" style="margin-top:10px"><button type="button" class="btn soft" data-copy-recovery-plan>Copy recovery plan</button></div><p>No recovery has occurred. Sync remains paused and Recovery Mode remains on.</p></div>`;
}
