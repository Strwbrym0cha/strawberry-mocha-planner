...
export async function buildRecoveryPlan({local,cloud,snapshot,snapshotRevision=4,buildVersion='unknown',sourceFingerprintsVerified=true}={}){
...
export async function collectReadOnlyRecoveryPlan({storage=localStorage,engine,snapshotRevision=4,expectedCloudRevision=null,expectedLocalHash=null,expectedCloudHash=null,expectedSnapshotHash=null,buildVersion='unknown'}={}){
  ...
  return buildRecoveryPlan({local,cloud:cloudResult.envelope,snapshot:snapshotResult.envelope,snapshotRevision,buildVersion,sourceFingerprintsVerified:true});
}
