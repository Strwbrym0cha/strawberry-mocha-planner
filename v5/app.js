...
function render(){
  ...
  const content=ui.view==='boss'?renderBoss(snapshot,ui.bossLane):renderRoom(ui.view,snapshot);
  document.body.className=`mode-${ui.mode}`;
  app.className='katos';
  app.innerHTML=`${sidebar()}${topbar()}<main class="main">${content}</main>`;
}
