import{loadV5Ui,saveV5Ui,saveV5DailyNote,saveV5RoomDetail,saveV5LedgerEntry,saveV5Workspace,updateV5Record,archiveV5Record,openV5DayReview,snapshotV4,migrateV4ToV5,restoreCloudV4Data,importV4Export}from'./data.js?v=5.0.22-monthly-bills';
import{renderBoss}from'./boss.js?v=5.0.22-monthly-bills';
import{renderRoom}from'./rooms.js?v=5.0.22-monthly-bills';
...
function render(){const oldSidebar=app.querySelector('.sidebar'),scrollTop=oldSidebar?.scrollTop||0,scrollLeft=oldSidebar?.scrollLeft||0,snapshot=snapshotV4(),content=ui.view==='boss'?renderBoss(snapshot,ui.bossLane):renderRoom(ui.view,snapshot,{scheduleView:ui.scheduleView});document.body.className=`mode-${ui.mode}`;app.className='katos';app.innerHTML=`${sidebar()}${topbar()}<main class="main">${content}</main>`;[...app.querySelectorAll('a[href*="../v4"]')].forEach(link=>{const card=link.closest('.card');if(card&&card.querySelectorAll('a[href*="../v4"]').length===1&&card.querySelectorAll('.btn, a.quick-action').length===1)card.remove();else link.remove()});const workspace=app.querySelector('main .detail-room[data-detail-room]');if(workspace&&ui.view!=='money'){const add=workspace.querySelector('[data-detail-open]'),targets=[...app.querySelectorAll('main .grid .card .card-head')];if(add&&targets.length){targets.forEach((target,index)=>{const button=index===0?add:add.cloneNode(true),actions=target.querySelector('.card-head-actions');(actions||target).append(button)});workspace.remove()}}const newSidebar=app.querySelector('.sidebar');if(newSidebar){newSidebar.scrollTop=scrollTop;newSidebar.scrollLeft=scrollLeft}}
...
function optionsFor(path,key,value){
  ...
  const moneyChoices={
    'money.bills.category':MONEY_CATEGORIES,'money.bills.repeat':['Monthly','Weekly','Every two weeks','Yearly','One time'],'money.bills.dueDay':Array.from({length:31},(_,index)=>String(index+1)),'money.bills.status':['Due','Paid','Skipped'],
