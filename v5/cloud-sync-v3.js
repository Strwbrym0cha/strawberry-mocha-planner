...
const PROTECTED_KEYS=new Set([V5_DATA_KEY,'sm_v4_beta','sm_v16_session','sm_cloud_session','sb-sigjwmgekmrwehylvuvu-auth-token',DEVICE_KEY,'sm_recovery_lock',...AUX_KEYS]);
const BACKUP_RE=/(backup|before|restore|recovery|cloud-pull|preimport)/i;
...
function backupKeys(){const rows=[];try{for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i);if(!key||PROTECTED_KEYS.has(key)||!BACKUP_RE.test(key))continue;const raw=localStorage.getItem(key)||'';rows.push({key,raw,size:raw.length});}}catch{}return rows;}
function pruneExactDuplicateBackups(){const seen=new Map();let freed=0;for(const row of backupKeys()){if(!row.raw)continue;if(seen.has(row.raw)){try{localStorage.removeItem(row.key);freed+=row.size}catch{}}else seen.set(row.raw,row.key)}return freed;}
function pruneLargestBackupArtifacts(bytesNeeded=0){const rows=backupKeys().sort((a,b)=>b.size-a.size);let freed=0;for(const row of rows){try{localStorage.removeItem(row.key);freed+=row.size}catch{}if(freed>=Math.max(bytesNeeded*2,256*1024))break}return freed;}
function safeSet(key,value){try{localStorage.setItem(key,value);return true}catch(error){if(!isQuota(error))throw error;pruneExactDuplicateBackups();try{localStorage.setItem(key,value);return true}catch(second){if(!isQuota(second))throw second;pruneLargestBackupArtifacts(String(value).length);localStorage.setItem(key,value);return true}}}
function localizeCanonical(data){let state;try{state=typeof structuredClone==='function'?structuredClone(data):JSON.parse(JSON.stringify(data))}catch{state={...data}}const aux=isObject(state?.__katosAuxStores)?state.__katosAuxStores:null;if(state&&typeof state==='object')delete state.__katosAuxStores;return{state,aux}}
function restoreAuxStores(aux){if(!isObject(aux))return;for(const key of AUX_KEYS){if(!(key in aux))continue;const raw=typeof aux[key]==='string'?aux[key]:JSON.stringify(aux[key]);try{safeSet(key,raw)}catch(error){console.warn(`KatOS could not restore ${key} yet.`,error)}}}
function applyCanonical(row){if(!row?.data)return false;const {state,aux}=localizeCanonical(row.data);restoreAuxStores(aux);const raw=JSON.stringify(state);safeSet(V5_DATA_KEY,raw);try{localStorage.setItem('sm_cloud_canonical_revision',String(Number(row.revision||0)));localStorage.setItem('sm_cloud_canonical_active','1')}catch{}baselineRaw=raw;currentRevision=Number(row.revision||0);canonicalReady=true;window.dispatchEvent(new CustomEvent('katos:cloud-sync',{detail:{status:'pulled',revision:currentRevision,readOnly:true,acceptedRecovery:row.last_device_id==='accepted-recovery'}}));return true}
...
function startAutoSync(){baselineRaw=localStorage.getItem(V5_DATA_KEY)||baselineRaw;clearInterval(pullTimer);pullTimer=setInterval(async()=>{if(busy||document.visibilityState==='hidden'||!canonicalReady)return;const session=await validSession();if(!session)return;busy=true;try{const row=await fetchCanonical(session);if(row&&Number(row.revision||0)>currentRevision){applyCanonical(row);location.reload()}}catch(error){console.warn('KatOS cloud refresh paused.',error)}finally{busy=false}},PULL_INTERVAL_MS)}
export async function prepareCloudSync(){const session=await validSession();if(!session)return{ok:false,offline:true};const row=await fetchCanonical(session);if(row){applyCanonical(row);startAutoSync();return{ok:true,action:'pulled',revision:currentRevision,readOnly:true}}canonicalReady=false;return{ok:true,action:'none',readOnly:true}}
export async function syncNow(){const session=await validSession();if(!session)return{ok:false,error:'Sign in to KatOS first.'};const row=await fetchCanonical(session);if(!row)return{ok:true,action:'none',readOnly:true};applyCanonical(row);return{ok:true,action:'pulled',revision:currentRevision,readOnly:true}}
export async function useThisDeviceAsCloud(){throw new Error('Cloud writes are temporarily locked while KatOS recovery is being verified. Your recovered cloud copy is protected.')}
export async function signInAndLoad(email,password){const session=await signIn(email,password);const row=await fetchCanonical(session);if(row){applyCanonical(row);startAutoSync();return{action:'pulled',revision:currentRevision,readOnly:true}}return{action:'none',readOnly:true}}
export function cloudSignedIn(){return !!readStoredSession()}
export function cloudSignOut(){clearSession();canonicalReady=false;clearInterval(pullTimer)}
