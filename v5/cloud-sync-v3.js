...
function restoreAuxStores(state){const aux=state?.__katosAuxStores;if(!isObject(aux))return;for(const key of AUX_KEYS){if(!(key in aux))continue;try{localStorage.setItem(key,typeof aux[key]==='string'?aux[key]:JSON.stringify(aux[key]))}catch{}}}
function applyCanonical(row){if(!row?.data)return false;restoreAuxStores(row.data);const raw=JSON.stringify(row.data);localStorage.setItem(V5_DATA_KEY,raw);baselineRaw=raw;currentRevision=Number(row.revision||0);canonicalReady=true;window.dispatchEvent(new CustomEvent('katos:cloud-sync',{detail:{status:'pulled',revision:currentRevision,readOnly:true}}));return true}
