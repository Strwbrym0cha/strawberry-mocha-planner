...
function patchGoalForms(){
 ...
 for(const form of app.querySelectorAll('[data-money-form="gig-goal-save"]')){
  ...
  if(!form.elements.currentAmount){const fields=form.querySelector('.room-detail-fields');fields?.insertAdjacentHTML('beforeend',`<label class="money-field"><span>Amount toward goal now</span><input name="currentAmount" type="number" min="0" step="0.01" inputmode="decimal" value="${esc(runway?.earned||0)}"></label><p class="money-form-note wide">Completed DoorDash and Flex summaries keep counting automatically. Change this number when you need to correct the total without editing old shifts.</p>`)}
