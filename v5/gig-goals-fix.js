...
function runwayFor(snapshot,view,goal){const progress=getGigGoalProgress(snapshot.state,goal.id,snapshot.today)||{goal,earned:0,range:{from:goal.startDate,to:goal.endDate}},earned=Math.max(0,cents(Number(progress.earned||0)+Number(goal.progressAdjustment||0)));return buildGigGoalRunway({goal,progress:{...progress,earned},shifts:shiftsFrom(snapshot),platforms:view.gig?.platforms,today:snapshot.today})}
...
function mountGoalList(){
 ...
 const goalStat=app.querySelector('.stat-grid .stat:nth-child(2)');if(goalStat&&runway)goalStat.innerHTML=`<small>GOAL</small><b>${Math.round(runway.coveredPercent)}%</b><span>${money(runway.unplanned)} still unplanned</span>`;
...
function patchGoalForms(){
 const snapshot=snapshotV4(),view=selectV5MoneyGig(snapshot.today);
 for(const form of app.querySelectorAll('[data-money-form="gig-goal-save"]')){
  const id=form.dataset.moneyId,goal=id?list(view.gig?.goals).find(row=>String(row.id)===String(id)):null,runway=goal?runwayFor(snapshot,view,goal):null;
  const target=form.elements.targetAmount;if(target&&Number(target.value)===0)target.value='';const name=form.elements.name;if(name&&!name.placeholder)name.placeholder='Ex: Earn $1,500 for bills';
  const fields=form.querySelector('.room-detail-fields'),accounts=list(view.hq?.accounts).filter(row=>row.active!==false),linked=text(goal?.linkedAccountId);
  if(!form.elements.linkedAccountId)fields?.insertAdjacentHTML('beforeend',`<label class="money-field wide"><span>Account holding this goal · optional</span><select name="linkedAccountId"><option value="">Track net earnings only</option>${accounts.map(account=>`<option value="${esc(account.id)}"${String(account.id)===linked?' selected':''}>${esc(account.name||'Account')} · ${money(view.accountBalances?.[account.id]?.posted||0)}</option>`).join('')}</select></label><p class="money-form-note wide">When linked, the posted balance in this account becomes the amount you actually have toward the goal. Earned and scheduled gig money stay separate until it reaches the account.</p>`);
  if(!form.elements.currentAmount)fields?.insertAdjacentHTML('beforeend',`<label class="money-field"><span>Earned amount correction</span><input name="currentAmount" type="number" min="0" step="0.01" inputmode="decimal" value="${esc(runway?.earned||0)}"></label><p class="money-form-note wide">Completed DoorDash and Flex summaries count automatically. This correction changes earned history only; it never changes a linked account balance.</p>`)
  if(id&&!form.querySelector('[data-gig-goal-archive]'))form.insertAdjacentHTML('beforeend',`<button type="button" class="btn soft gig-goal-archive" data-gig-goal-archive="${esc(id)}">Archive goal</button>`);
  if(!id)syncDates(form);
 }
}

app?.addEventListener('click',event=>{const add=event.target.closest?.('[data-money-open="new-gig-goal"]');if(add&&app.contains(add)){event.preventDefault();event.stopImmediatePropagation();patchGoalForms();showModal(modalFor('new-gig-goal'));return}const edit=event.target.closest?.('[data-gig-goal-edit]');if(edit&&app.contains(edit)){event.preventDefault();event.stopImmediatePropagation();patchGoalForms();showModal(modalFor(`gig-goal-${edit.dataset.gigGoalEdit}`));return}const archive=event.target.closest?.('[data-gig-goal-archive]');if(archive&&app.contains(archive)){event.preventDefault();event.stopImmediatePropagation();if(!window.confirm('Archive this gig goal? It will move to Memory Box, not be deleted.'))return;const result=archiveV5Record('work.gig.goals',archive.dataset.gigGoalArchive);if(!result.ok){window.alert(result.error||'That gig goal could not be archived.');return}archive.closest('[data-money-modal]')?.remove();document.querySelector('[data-view="boss"]')?.click();setTimeout(mountGoalList,50);return}},true);
