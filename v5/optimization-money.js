...
import{addDays,buildShiftScorecards,calculateSafeToSpend}from'./optimization-core.js?v=7.4.0-intelligence-pass';
...
function renderGigScores(){
 ...
 const state=obj(snapshotV4().state),rows=buildShiftScorecards(state?.work?.gigShifts,state?.work?.gig?.orders,state?.money?.hq?.transactions),node=document.createElement('section');node.className='card full money-card optimization-gig-scores';node.dataset.optimizationGigScores='';
 const cards=rows.map(row=>{
  ...
  const workload=row.kind==='Amazon Flex'?row.units+' packages • '+row.stops+' stops':row.units+' deliveries',rates=row.kind==='Amazon Flex'&&row.packagesPerHour!=null?row.packagesPerHour+' pkg/hr • '+row.stopsPerHour+' stops/hr':(row.mileage?row.mileage+' miles':'miles not entered');
  return'<article class="gig-score"><header><span>'+row.icon+'</span><div><b>'+esc(row.kind)+'</b><small>'+esc(formatDate(row.date))+'</small></div><em class="'+(row.summaryComplete?'done':'pending')+'">'+(row.summaryComplete?'Summary saved':'Summary needed')+'</em></header><div class="gig-score-numbers"><div><small>GROSS</small><b>'+money(row.actual)+'</b><span>'+duration(row.actualMinutes)+'</span></div><div><small>EXPENSES</small><b>'+money(row.expenses)+'</b><span>'+esc(workload)+'</span></div><div><small>NET PROFIT</small><b>'+money(row.net)+'</b><span>'+rates+'</span></div><div><small>NET RATE</small><b>'+(row.perHour==null?'—':money(row.perHour)+'/hr')+'</b><span>'+(row.perMile==null?'miles not entered':money(row.perMile)+'/mi')+'</span></div></div><footer><span>'+esc(pay)+'</span><span>'+esc(time)+'</span></footer></article>';
 }).join('');
 ...
 const platformComparison=['Amazon Flex','DoorDash'].map(kind=>{const matching=rows.filter(row=>row.kind===kind),gross=matching.reduce((sum,row)=>sum+row.actual,0),expenses=matching.reduce((sum,row)=>sum+row.expenses,0),net=matching.reduce((sum,row)=>sum+row.net,0),minutes=matching.reduce((sum,row)=>sum+row.actualMinutes,0),miles=matching.reduce((sum,row)=>sum+row.mileage,0);return{kind,count:matching.length,gross,expenses,net,perHour:minutes?net/(minutes/60):null,perMile:miles?net/miles:null}}),comparison='<div class="gig-profit-comparison">'+platformComparison.map(row=>'<div><small>'+esc(row.kind.toUpperCase())+' · '+row.count+' SHIFT'+(row.count===1?'':'S')+'</small><b>'+money(row.net)+' net</b><span>'+money(row.gross)+' gross − '+money(row.expenses)+' costs</span><em>'+(row.perHour==null?'No completed hours':money(row.perHour)+'/hr')+(row.perMile==null?'':' · '+money(row.perMile)+'/mi')+'</em></div>').join('')+'</div>';
 node.innerHTML='<div class="optimization-score-summary"><div><div class="ey">📊 ACTUAL PROFITABILITY</div><h2>Flex vs DoorDash</h2><p>'+esc(summary)+'</p></div><button type="button" class="btn soft optimization-score-open" data-money-open="optimization-shift-scorecards">Open scorecards <span aria-hidden="true">→</span></button></div>'+comparison+'<div class="detail-modal-backdrop money-modal optimization-score-modal" data-money-modal="optimization-shift-scorecards" hidden><section class="detail-modal" role="dialog" aria-modal="true" aria-labelledby="optimization-score-title"><div class="detail-modal-head"><div><div class="ey">📊 SHIFT SCORECARDS</div><h2 id="optimization-score-title">Which shifts were actually worth it?</h2><p>Gross pay, linked ledger costs, net profit, time, mileage, and workload stay together.</p></div><button type="button" class="detail-modal-close" data-money-close aria-label="Close scorecards">×</button></div><div class="gig-score-list">'+(cards||'<div class="empty">Finish one planned shift and its scorecard will appear here.</div>')+'</div><div class="button-row optimization-score-footer"><button type="button" class="btn soft" data-money-close>Done</button></div></section></div>';
