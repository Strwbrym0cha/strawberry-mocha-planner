...
app.addEventListener('focusin',event=>{const form=event.target.closest?.('[data-flex-shift-form]');if(form&&['actualHours','actualMinutesPart'].includes(event.target.name))event.target.select?.()});
