...
function routineOccurs(routine,date){return routine?.archived!==true&&routine?.active!==false&&!routinePaused(routine,date)&&(recurrenceOf(routine).kind==='none'||occursOn(routine,date))}
function stepId(routine,step,index){return text(obj(step).id)||`${routine.id}-step-${index}`}
function routineSteps(routine){return list(routine?.steps).map((step,index)=>typeof step==='string'?{id:stepId(routine,step,index),label:step,minutes:5}:{...step,id:stepId(routine,step,index),label:text(step.label||step.text||step.title),minutes:Math.max(0,Number(step.minutes)||5)}).filter(step=>step.label)}
function startOfWeek(date){const value=new Date(`${date}T12:00:00`);value.setDate(value.getDate()-((value.getDay()+6)%7));return value}
function dateKey(value){return`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,'0')}-${String(value.getDate()).padStart(2,'0')}`}
function routineInstanceStatus(instance,steps){if(!instance)return'not-logged';if(instance.status==='skipped')return'skipped';if(instance.status==='complete')return'complete';const values=steps.map(step=>text(obj(instance.steps)[step.id])),complete=values.filter(value=>value==='complete').length,resolved=values.filter(value=>['complete','skipped'].includes(value)).length;if(steps.length&&complete===steps.length)return'complete';if(complete||resolved||instance.tinyStart==='complete')return'partial';return instance.status==='running'?'running':'not-logged'}
function plannedDates(routine,date){const start=startOfWeek(date);return Array.from({length:7},(_,index)=>{const day=new Date(start);day.setDate(start.getDate()+index);return dateKey(day)}).filter(key=>key<=date&&routineOccurs(routine,key))}
function routineHistory(state,routine,date){const steps=routineSteps(routine),rows=routineInstances(state).filter(item=>String(item?.routineId)===String(routine?.id)&&dateOk(item?.date)&&item.date<=date).map(instance=>{const values=steps.map(step=>text(obj(instance.steps)[step.id])),complete=values.filter(value=>value==='complete').length,skipped=values.filter(value=>value==='skipped').length,status=routineInstanceStatus(instance,steps);return{date:instance.date,status,complete,skipped,total:steps.length,ratio:steps.length?Math.round(complete/steps.length*100):0,instance}}).sort((a,b)=>b.date.localeCompare(a.date));const weekDates=new Set(plannedDates(routine,date)),weekStart=dateKey(startOfWeek(date)),week=rows.filter(row=>row.date>=weekStart&&row.date<=date),weekCompleted=week.filter(row=>row.status==='complete').length,weekPartial=week.filter(row=>row.status==='partial').length,planned=weekDates.size,logged=week.filter(row=>row.status!=='not-logged').length,opportunities=Math.max(planned,logged),recent=rows[0]?.date===date?rows.slice(0,7):[{date,status:'not-logged',complete:0,skipped:0,total:steps.length,ratio:0,instance:null},...rows].slice(0,7);return{recent,weekCompleted,weekPartial,weekPlanned:planned,weekLogged:logged,consistency:opportunities?Math.round((weekCompleted+weekPartial*.5)/opportunities*100):0}}
function routineView(state,routine,date){const instance=routineInstance(state,routine,date),steps=routineSteps(routine),statuses=obj(instance?.steps),complete=steps.filter(step=>text(statuses[step.id])==='complete').length,stepSkipped=steps.filter(step=>text(statuses[step.id])==='skipped').length,resolved=complete+stepSkipped,tinyStart=text(routine.tinyStart||routine.firstStep||steps[0]?.label),status=routineInstanceStatus(instance,steps);return{id:String(routine.id),date,title:titleOf(routine),icon:text(routine.icon)||'🍓',category:text(routine.category)||'Self care',daypart:text(routine.daypart||routine.preferredDaypart),preferredTime:timeOf(routine),effort:text(routine.effort||routine.estimatedEffort)||'medium',notes:text(routine.notes),active:routine.active!==false,reminderEnabled:routine.reminderEnabled===true||routine.reminder==='on',reminderTime:text(routine.reminderTime),routine,instance,steps,tinyStart,tinyStartDone:instance?.tinyStart==='complete',status,resolved,complete,stepSkipped,total:steps.length,remaining:steps.length-resolved,paused:routinePaused(routine,date),skipped:status==='skipped',history:routineHistory(state,routine,date)};}
...
export function selectDailyShit(state={},date,{mode='normal'}={}){
 ...
 return{date,mode,rightNow:recommendation,today,timed,could,later,done,routines,routineLibrary,overdue,tired,open};
}
...
export function applyDailyAction(source,action={},today){const state=clone(source),type=text(action.type),date=dateOk(action.date)?action.date:today;if(!dateOk(today))return fail('A local planner date is required.');
 ...
 if(type==='quick-add'){
  ...
  if(kind==='routine'){const recurrence=recurrenceOf({recurrence:action.recurrence||'daily',repeatDays:action.repeatDays}),steps=text(action.steps).split(/\r?\n/).map(label=>text(label)).filter(Boolean).map((label,index)=>({id:makeId('routine-step'),label,minutes:5,essential:index===0})),tinyStart=text(action.tinyStart),routine={id:makeId('routine'),name:title,icon:text(action.icon)||'🍓',category:text(action.category)||'Self care',recurrence,daypart:text(action.daypart),preferredTime:text(action.preferredTime),active:action.active!==false&&action.active!=='false',reminderEnabled:action.reminderEnabled===true||action.reminderEnabled==='on',reminderTime:text(action.reminderTime),tinyStart,effort:text(action.effort)||'medium',notes:text(action.notes),steps,archived:false,createdAt:new Date().toISOString()};atPath(state,'life.routines',[...items(state,'life.routines'),routine]);return{ok:true,state,result:routine};}
  const path=kind==='ping'?'life.reminders':'life.tasks';const item={id:makeId(kind),[kind==='task'?'text':'title']:title,title,date:action.forToday===false?'':date,time:text(action.time),priority:text(action.priority)||'normal',energy:text(action.energy)||'medium',minutes:Number(action.duration)||0,deadlineType:action.deadlineType==='hard'?'hard':'soft',done:false,createdAt:new Date().toISOString()};atPath(state,path,[...items(state,path),item]);return{ok:true,state,result:item};
 }
 ...
 if(type.startsWith('routine-')){
  ...
  if(type==='routine-step'){const index=Number(action.index),steps=routineSteps(routine),step=steps[index];if(!step)return fail('That routine step is no longer here.');const instance=createRoutineInstance(state,routine,date,{status:'running'}),status=['complete','skipped','later','open'].includes(action.status)?action.status:'complete',nextSteps={...obj(instance.steps),[step.id]:status},values=steps.map(item=>text(nextSteps[item.id])),complete=values.filter(value=>value==='complete').length,resolved=values.filter(value=>['complete','skipped'].includes(value)).length,nextStatus=steps.length&&complete===steps.length?'complete':complete||resolved||instance.tinyStart==='complete'?'partial':'running';return{ok:true,state,result:createRoutineInstance(state,routine,date,{...instance,steps:nextSteps,status:nextStatus,completedAt:nextStatus==='complete'?new Date().toISOString():''})};}
  if(type==='routine-complete'){const steps=routineSteps(routine),instance=createRoutineInstance(state,routine,date,{status:'running'}),values=steps.map(step=>text(obj(instance.steps)[step.id])),complete=values.filter(value=>value==='complete').length,skipped=values.filter(value=>value==='skipped').length,status=steps.length&&complete===steps.length?'complete':complete||skipped||instance.tinyStart==='complete'?'partial':'not-logged';if(status==='not-logged')return fail('Complete the Tiny Start or a routine step first.');return{ok:true,state,result:createRoutineInstance(state,routine,date,{...instance,status,completedSteps:complete,totalSteps:steps.length,completionRatio:steps.length?complete/steps.length:0,loggedAt:new Date().toISOString(),completedAt:status==='complete'?new Date().toISOString():''})};}
  ...
  if(type==='routine-update-step'){const index=Number(action.index),label=text(action.label),steps=routineSteps(routine);if(!steps[index])return fail('That routine step is no longer here.');if(!label)return fail('A routine step needs a name.');steps[index]={...steps[index],label};return{ok:true,state,result:patchItem(state,'life.routines',routine.id,{...routine,steps})};}
  if(type==='routine-remove-step'){const index=Number(action.index),steps=routineSteps(routine);if(!steps[index])return fail('That routine step is no longer here.');return{ok:true,state,result:patchItem(state,'life.routines',routine.id,{...routine,steps:steps.filter((_,stepIndex)=>stepIndex!==index)})};}
  ...
  if(type==='routine-update'){const name=text(action.name);if(!name)return fail('A routine needs a name.');const recurrence=recurrenceOf({recurrence:action.recurrence,repeatDays:action.repeatDays});return{ok:true,state,result:patchItem(state,'life.routines',routine.id,{...routine,name,icon:text(action.icon)||routine.icon||'🍓',category:text(action.category),recurrence,daypart:text(action.daypart),preferredTime:text(action.preferredTime),active:action.active===true||action.active==='on'||action.active==='true',reminderEnabled:action.reminderEnabled===true||action.reminderEnabled==='on',reminderTime:text(action.reminderTime),tinyStart:text(action.tinyStart),effort:text(action.effort)||'medium',notes:text(action.notes),pausedUntil:action.pausedUntil||''})};}
