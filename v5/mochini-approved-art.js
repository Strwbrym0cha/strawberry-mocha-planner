...
const modeStyle=(el,mode)=>{if(!el)return;const order=['home','daily','schedule','work','money','study','pings','gig','career','settings'],key=order.includes(mode)?mode:'home',index=order.indexOf(key),column=index%5,row=Math.floor(index/5),anchor=FACE_ANCHORS[key]||FACE_ANCHORS.home,rig=el.parentElement,changed=el.dataset.mode&&el.dataset.mode!==key;el.dataset.mode=key;el.style.backgroundImage=`url(${MODE_ATLAS})`;el.style.backgroundSize='500% 200%';el.style.backgroundPosition=`${column*25}% ${row*100}%`;rig?.style.setProperty('--mc-face-x',`${anchor[0]*100}%`);rig?.style.setProperty('--mc-face-y',`${anchor[1]*100}%`);if(changed&&rig){rig.classList.remove('is-mode-switching');void rig.offsetWidth;rig.classList.add('is-mode-switching');setTimeout(()=>rig.classList.remove('is-mode-switching'),260)}};
