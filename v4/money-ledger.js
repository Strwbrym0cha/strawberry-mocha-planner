...
function summary(state){
 ...
 const unpaid=list(state?.money?.bills).filter(b=>b?.paid!==true).reduce((sum,b)=>sum+money(b.amount),0);
...
function transactionMarkup(s){
 ...
 <div class="money-ledger-stats"><div class="money-ledger-stat"><small>AVAILABLE NOW</small><b>${currency(n.available)}</b><span>${n.all.length?'account bases + ledger':'ledger net'}</span></div><div class="money-ledger-stat"><small>SAFE AFTER BILLS</small><b>${currency(n.safe)}</b><span>${currency(n.unpaid)} unpaid bills</span></div><div class="money-ledger-stat"><small>IN THIS MONTH</small><b>${currency(n.inMonth)}</b><span>income recorded</span></div><div class="money-ledger-stat"><small>SPENT THIS MONTH</small><b>${currency(n.spent)}</b><span>from your ledger</span></div></div>
