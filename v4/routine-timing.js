...
function daypartAvailable(daypart,now=new Date()){
  ...
  if(part==='morning')return inWindow(mins,300,720);
  if(part==='daytime'||part==='day')return inWindow(mins,600,1080);
  if(part==='evening')return inWindow(mins,990,1320);
  if(part==='bedtime'||part==='night'||part==='nighttime')return inWindow(mins,1200,180);
  return true;
}
function routineAvailableNow(r,now=new Date()){
  const target=minutesOfTime(r?.preferredTime||r?.targetTime||r?.time||r?.startTime);
...
function timeLabel(r){const pref=text(r.preferredTime||r.targetTime||r.time||r.startTime);if(pref){try{return new Date(`2000-01-01T${pref}:00`).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}catch{return pref}}const part=text(r.daypart)||'any';return part==='any'?'Any time':part[0].toUpperCase()+part.slice(1)}
...
.routine-timing-manager{grid-column:1/-1}.routine-timing-list{display:grid;gap:7px;margin-top:10px}.routine-timing-row{display:grid;grid-template-columns:minmax(150px,1.3fr) minmax(120px,.7fr) minmax(120px,.7fr) auto;gap:8px;align-items:end;padding:10px;border:1px solid #ead7df;border-radius:14px;background:#fff}.routine-timing-row b{display:block}.routine-timing-row small{display:block;color:#91747f;margin-top:2px}.routine-timing-row label{display:grid;gap:4px;font-size:9px;font-weight:850;color:#765863}.routine-timing-row input,.routine-timing-row select{width:100%;padding:8px 9px;border:1px solid #e5ced7;border-radius:10px;background:#fff;font:inherit}.routine-timing-actions{display:flex;gap:5px;flex-wrap:wrap;align-items:center;justify-content:flex-end}.routine-waiting-note{margin-top:9px;padding:8px 10px;border-radius:12px;background:#fff8fb;color:#8d6f7a;font-size:10px}.routine-time-hint{font-size:10px;color:#8d707a;margin-top:5px}@media(max-width:760px){.routine-timing-row{grid-template-columns:1fr 1fr}.routine-timing-actions{grid-column:1/-1;justify-content:flex-start}}@media(max-width:520px){.routine-timing-row{grid-template-columns:1fr}}
`;document.head.appendChild(s)}

function enhanceCreateForm(){
  const form=document.querySelector('form[data-form="routine"]');if(!form)return;
  // The core creator owns these fields. This enhancement only claims its submit
  // path after confirming the unified form is present; it never injects fields.
  if(form.querySelector('[name="preferredTime"]')){form.dataset.routineTimingEnhanced='1';syncSelectedDays(form)}
}
function syncSelectedDays(form){const recurrence=form.querySelector('[name="recurrence"]'),days=form.querySelector('[name="days"]')?.closest('.field');if(!recurrence||!days)return;days.style.opacity=recurrence.value==='selected'?'1':'.56'}
function managerMarkup(state){
  const rows=list(state.life?.routines).filter(r=>!r.archived&&!store.isArchived(state,'routine',r.id));
  return`<section class="card full routine-timing-manager" data-routine-timing-manager><div class="ey">⏰ ROUTINE TIMING + CONTROL</div><h2>When should these actually bother me?</h2><p>Daypart is the fallback. Add an exact preferred time when you want a tighter window. Test routine? Delete her with zero ceremony.</p><div class="routine-timing-list">${rows.length?rows.map(r=>`<div class="routine-timing-row"><div><b>${esc(r.name||r.title||'Routine')}</b><small>Currently: ${esc(timeLabel(r))}</small></div><label>Daypart<select data-routine-daypart="${esc(r.id)}">${['morning','daytime','evening','bedtime','any'].map(p=>`<option value="${p}" ${text(r.daypart||'any')===p?'selected':''}>${p}</option>`).join('')}</select></label><label>Preferred time<input data-routine-preferred="${esc(r.id)}" type="time" value="${esc(text(r.preferredTime||r.targetTime||r.time||r.startTime||''))}"></label><div class="routine-timing-actions"><button class="btn tiny" data-routine-timing-save="${esc(r.id)}">Save timing</button><button class="btn tiny" data-routine-archive="${esc(r.id)}">📦 Archive</button><button class="btn tiny danger" data-routine-delete="${esc(r.id)}">× Delete</button></div></div>`).join(''):'<div class="empty">No routines yet.</div>'}</div></section>`
}
...
function saveSchedule(button){const form=button.closest('form[data-routine-schedule-edit]'),id=button.dataset.routineScheduleSave;if(!form||!id)return;const fd=new FormData(form),state=clone(rt.getState()),routine=list(state.life?.routines).find(r=>String(r.id)===String(id));if(!routine)return;routine.recurrence=text(fd.get('recurrence'))||'daily';routine.days=fd.getAll('days').map(Number).filter(day=>Number.isInteger(day)&&day>=0&&day<=6);routine.preferredTime=text(fd.get('preferredTime'));routine.updatedAt=new Date().toISOString();rt.setState(state,'Routine schedule updated')}
function archiveRoutine(id){let state=clone(rt.getState());const r=list(state.life?.routines).find(x=>String(x.id)===String(id));if(!r)return;state=store.archiveItem(state,'routine',id);rt.setState(state,`${r.name||r.title||'Routine'} archived 📦`)}
function deleteRoutine(id){let state=clone(rt.getState());const r=list(state.life?.routines).find(x=>String(x.id)===String(id));if(!r)return;const name=r.name||r.title||'this routine';if(!confirm(`Delete “${name}” for real?\n\nThis also removes its saved daily instances. Archive is safer if you might want it back.`))return;state.life.routines=list(state.life?.routines).filter(x=>String(x.id)!==String(id));state.life.routineInstances=list(state.life?.routineInstances).filter(x=>String(x.routineId)!==String(id));state.v4={...(state.v4||{}),archive:list(state.v4?.archive).filter(a=>!(a.kind==='routine'&&String(a.id)===String(id))),archivedRecords:list(state.v4?.archivedRecords).filter(a=>!(a.kind==='routine'&&String(a.record?.id)===String(id)))};rt.setState(state,`${name} deleted`)}
function saveNewRoutine(form){const fd=new FormData(form),state=clone(rt.getState()),steps=text(fd.get('steps')).split('\n').map((label,i)=>({id:makeId(`step${i}`),label:text(label),minutes:5,optional:false})).filter(x=>x.label),days=fd.getAll('days').map(Number).filter(day=>Number.isInteger(day)&&day>=0&&day<=6);state.life.routines=[...list(state.life?.routines),{id:makeId('routine'),name:text(fd.get('name')),icon:text(fd.get('icon'))||'🎀',daypart:text(fd.get('daypart'))||'any',preferredTime:text(fd.get('preferredTime')),recurrence:text(fd.get('recurrence'))||'daily',days,steps,archived:false,createdAt:new Date().toISOString()}];rt.setState(state,'Routine added')}
...
document.addEventListener('change',e=>{const recurrence=e.target.closest?.('form[data-form="routine"] [name="recurrence"]');if(recurrence)syncSelectedDays(recurrence.closest('form'))});
document.addEventListener('click',e=>{const schedule=e.target.closest?.('[data-routine-schedule-save]');if(schedule){saveSchedule(schedule);return}const save=e.target.closest?.('[data-routine-timing-save]');if(save){saveTiming(save.dataset.routineTimingSave);return}const archive=e.target.closest?.('[data-routine-archive]');if(archive){archiveRoutine(archive.dataset.routineArchive);return}const del=e.target.closest?.('[data-routine-delete]');if(del)deleteRoutine(del.dataset.routineDelete)},true);
