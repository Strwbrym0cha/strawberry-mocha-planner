...
function ensureControl(){
  const foot=document.querySelector('.sidebar-foot');if(!foot)return;let wrap=document.getElementById('katos-recovery-control');if(!wrap){wrap=document.createElement('div');wrap.id='katos-recovery-control';wrap.innerHTML='<button type="button" data-katos-recovery-open><span>🛟 Restore V4 data</span><span>→</span></button>';foot.appendChild(wrap)}
}
