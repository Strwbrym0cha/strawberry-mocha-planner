...
function normalizeHobby(v,i=0){v=obj(v);return{...v,id:text(v.id)||`hobby-${i}`,name:text(v.name)||'Hobby',status:['playing','shelf','curious'].includes(v.status)?v.status:'playing',hobbyType:['creative','interactive','collecting'].includes(text(v.hobbyType))?text(v.hobbyType):'',kind:text(v.kind)||'general',lastTouched:text(v.lastTouched),notes:text(v.notes),supplies:text(v.supplies),cost:money(v.cost),skill:text(v.skill),language:v.language===true,createdAt:text(v.createdAt)||new Date().toISOString(),updatedAt:text(v.updatedAt)}}
