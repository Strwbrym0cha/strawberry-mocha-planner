...
export const V4_BUILD='4.0.0-preview.1';
...
function normalizeOpenPlan(v,i=0){v=obj(v);return{id:text(v.id)||`open-day-${i}`,date:text(v.date)||localDateKey(),items:list(v.items).map(x=>({id:text(x.id)||makeId('plan-item'),label:text(x.label),kind:text(x.kind)||'flex',done:x.done===true})),createdAt:text(v.createdAt)||new Date().toISOString()}}
...
function ensureMoneyLinks(state){const m=normalizeMoney(state.money),bills=[...m.bills],debts=m.debts.map(debt=>{
  ...
  if(!bill && amount>0 && (debt.dueDate||debt.dueDay)){
    bill=normalizeBill({id:makeId('bill'),name:`${debt.name} Payment`,amount,dueDate:debt.dueDate,dueDay:debt.dueDay,recurring:true,paid:false,linkedDebtId:debt.id,createdAt:new Date().toISOString()});
    bills.push(bill);
  }
...
function v4Defaults(){return{
  mode:'normal',
  energyBlocks:[normalizeBlock({name:'First wind',start:'08:00',end:'12:00',energy:'high'},0),normalizeBlock({name:'Second wind',start:'13:00',end:'17:00',energy:'okay'},1)],
  people:[],hobbies:[],admin:[],shopping:[],brainDump:[],openDayPlans:[],archive:[],
  ui:{lastView:'home',nomsTab:'fridge',moneyTab:'overview',memoryTab:'archive'}
}}

function importV3(){const v3=parseStorage(V3_KEY)||{};const now=new Date().toISOString();return normalizeState({...clone(v3),schemaVersion:4,meta:{...obj(v3.meta),build:V4_BUILD,createdAt:now,updatedAt:now,importedFromV3At:now},v4:v4Defaults()})}

export function normalizeState(value){const s=obj(value),life=obj(s.life),nourish=obj(s.nourish),insights=obj(s.insights),profile=obj(s.profile),prefs=obj(profile.preferences),legacyArchive=list(prefs.archiveRefs),v4={...v4Defaults(),...obj(s.v4)},meta=obj(s.meta);const next={
  ...s,
  schemaVersion:4,
  profile:{...profile,preferences:{...prefs,archiveRefs:[]}},
  context:{brain:'steady',energy:'okay',capacity:'normal',pressure:'chill',socialBattery:'neutral',mode:'normal',currentActivity:'',note:'',...obj(s.context)},
  life:{inbox:list(life.inbox),tasks:list(life.tasks),reminders:list(life.reminders),routines:list(life.routines),routineInstances:list(life.routineInstances),events:list(life.events),threads:list(life.threads)},
  nourish:{...nourish,noms:{foods:[],recipes:[],history:[],groceries:[],mealPlan:[],...obj(nourish.noms)},sips:{waterGoalOz:64,fridge:[],history:[],...obj(nourish.sips)}},
  movement:{sessions:[],routines:[],videos:[],weighIns:[],...obj(s.movement)},
  education:{programs:[],courses:[],items:[],sessions:[],reviews:[],...obj(s.education)},
  work:{items:[],schedule:{},shifts:[],training:[],career:[],focus:{},...obj(s.work)},
  money:normalizeMoney(s.money),
  growth:{goals:[],wins:[],experiments:[],...obj(s.growth)},
  insights:{activityLog:[],observations:[],experiments:[],resetSessions:[],legacyNotes:[],dayReviews:[],...insights},
  mochini:{conversation:[],pendingProposal:null,lastProposal:null,lastContextInference:null,...obj(s.mochini)},
  v4:{...v4,energyBlocks:list(v4.energyBlocks).map(normalizeBlock),people:list(v4.people).map(normalizePerson),hobbies:list(v4.hobbies).map(normalizeHobby),admin:list(v4.admin).map(normalizeAdmin),shopping:list(v4.shopping).map(normalizeWish),brainDump:list(v4.brainDump).map(normalizeDump),openDayPlans:list(v4.openDayPlans).map(normalizeOpenPlan),archive:normalizeArchive([...list(v4.archive),...legacyArchive]),ui:{lastView:'home',nomsTab:'fridge',moneyTab:'overview',memoryTab:'archive',...obj(v4.ui)}},
  meta:{...meta,build:V4_BUILD,createdAt:text(meta.createdAt)||new Date().toISOString(),updatedAt:text(meta.updatedAt)||new Date().toISOString()}
};return ensureMoneyLinks(next)}

export function loadState(){const existing=parseStorage(V4_KEY);const state=existing?normalizeState(existing):importV3();if(!existing)saveState(state);return state}
...
export function makeId(prefix){return makeId(prefix)}
...
export function upsertDebt(state,input){const next=clone(state),m=normalizeMoney(next.money),id=text(input.id)||makeId('debt');const prior=m.debts.find(x=>x.id===id);const debt=normalizeDebt({...prior,...input,id,createdAt:prior?.createdAt||new Date().toISOString()});m.debts=prior?m.debts.map(x=>x.id===id?debt:x):[...m.debts,debt];next.money=m;return ensureMoneyLinks(next)}
