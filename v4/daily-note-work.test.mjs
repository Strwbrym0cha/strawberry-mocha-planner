...
import{shiftMinutesForDate,ruleOccursOnDate,recurringShiftMinutesForDate,deriveWorkDefaults,displayWorkHours,upsertWorkRecap}from'./daily-note-work.js';
...
assert.equal(displayWorkHours(5,''),5);
assert.equal(displayWorkHours('5',0),5);
assert.equal(displayWorkHours('',3.5),3.5);
assert.equal(displayWorkHours(undefined,''),'');
...
assert.equal(displayWorkHours(next.insights.dayReviews[0].workHours,''),2.5);
