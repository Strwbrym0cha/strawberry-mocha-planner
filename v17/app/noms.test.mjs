...
const pantryBefore={...pantry.item};assert.equal(noms.linkedNomForPantry(state,pantry.item.id),null,'pantry stays pantry-only until Kat explicitly links it');
const pantryNom=actions.addNom({name:'Tortillas',effort:'no-prep'});assert.equal(pantryNom.ok,true);assert.equal(actions.linkPantryToNom(pantry.item.id,pantryNom.item.id).ok,true);assert.equal(noms.linkedNomForPantry(state,pantry.item.id).id,pantryNom.item.id);assert.equal(actions.linkPantryToNom(pantry.item.id,pantryNom.item.id).duplicate,true,'repeated linking does not create duplicate records');assert.equal(state.noms.pantry.find(item=>item.id===pantry.item.id).quantity,pantryBefore.quantity);assert.equal(state.noms.pantry.find(item=>item.id===pantry.item.id).unit,pantryBefore.unit);
...
assert.equal(actions.setTodayNom({text:'Freeform snack'}).ok,true);assert.equal(noms.todayNom(state).label,'Freeform snack');assert.equal(actions.setTodayNom({nomId:pantryNom.item.id}).ok,true);assert.equal(noms.todayNom(state).label,'Tortillas');assert.equal(actions.clearTodayNom().ok,true);
assert.equal(actions.archiveNom(nomId).ok,true);assert.equal(actions.archiveNom(pantryNom.item.id).ok,true);assert.equal(noms.allNoms(state).length,0,'archived Noms leave the active list without deleting saved records');assert.equal(state.noms.foods[0].archived,true);
