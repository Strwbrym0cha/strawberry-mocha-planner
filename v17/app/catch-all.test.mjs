...
const nomCapture=quickCapture(store,'Yogurt and granola');const nomRoute=routeCatchAllCapture(store,nomCapture.item.id,'nom',{name:'Yogurt and granola',effort:'no-prep'});assert.equal(nomRoute.ok,true);assert.equal(state.noms.foods[0].name,'Yogurt and granola');assert.equal(state.noms.foods[0].sourceCaptureId,nomCapture.item.id,'Nom keeps exact Catch-All provenance');
const groceryCapture=quickCapture(store,'Strawberry milk');const groceryRoute=routeCatchAllCapture(store,groceryCapture.item.id,'grocery',{name:'Strawberry milk'});assert.equal(groceryRoute.ok,true);assert.equal(state.noms.groceries[0].sourceCaptureId,groceryCapture.item.id,'Grocery keeps exact Catch-All provenance');assert.equal(undoCatchAllRoute(store,groceryCapture.item.id).ok,true);assert.equal(state.noms.groceries.length,0,'undo removes only the routed grocery');
