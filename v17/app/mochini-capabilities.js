...
const likelySweetFood=item=>hasTag(item,'sweet')||hasTag(item,'dessert')||/\b(ice cream|pancake|pineapple|candy|cookie|cake|brownie|donut|doughnut|chocolate|pie|fruit snack|pudding|cupcake)\b/.test(clean(title(item)));
const isSodaName=value=>/\b(soda|coke|coca cola|pepsi|dr pepper|drpepper|sprite|fanta|root beer|mountain dew)\b/.test(clean(value));
const isSweetDrinkName=value=>isSodaName(value)||/\b(juice|lemonade|sweet tea|gatorade|powerade|fruit punch)\b/.test(clean(value));
...
function nomsAnswer(input,state,date){
 ...
 const unique=available.filter((item,index,array)=>item&&array.findIndex(other=>String(other?.id)===String(item?.id))===index),fresh=unique.filter(item=>!consumedIds.has(String(item?.id||''))&&!consumedNames.has(clean(title(item)))),savoryRequested=/\b(savory|salty)\b/.test(input),sweetRequested=/\b(sweet|dessert)\b/.test(input),savoryFresh=fresh.filter(item=>hasTag(item,'savory')||(!likelySweetFood(item)&&!hasTag(item,'sweet'))),sweetFresh=fresh.filter(item=>likelySweetFood(item)),recommendations=savoryRequested?savoryFresh:sweetRequested?sweetFresh:fresh;
 ...
 if(!savoryRequested&&!sweetRequested&&planned?.label&&!quick&&!plannedAlreadyHad)return response('cap_noms_recommendation',history.length?`You’ve already logged ${history.map(item=>item.label).join(', ')} today. Your Meal Plan has a fresh option: ${planned.label}. 🍱`:`Your Meal Plan suggests ${planned.label} today. 🍱`,[planned,...history]);
 if(recommendations.length){const pick=recommendations[0],already=history.length?`You’ve already had ${history.map(item=>item.label).join(', ')}. `:'',lead=savoryRequested?'Savory pivot:':sweetRequested?'Sweet pivot:':quick?'For something quick, I’d grab':'Tiny bean food vote:';if(needsFoodNow(input))return response('cap_noms_recommendation',`${already}${lead} ${title(pick)}. It’s an option you haven’t logged yet today. 🍓`,[pick,...history],{signals:['food_now','not_already_eaten',savoryRequested?'savory':sweetRequested?'sweet':'general']});return response('cap_noms_recommendation',`${already}${lead} ${title(pick)}. It hasn’t shown up in today’s Nom Diary yet. 🍓`,[pick,...history],{signals:['not_already_eaten',savoryRequested?'savory':sweetRequested?'sweet':'general']});}
 if((savoryRequested||sweetRequested)&&fresh.length)return response('cap_noms_recommendation',`I heard ${savoryRequested?'savory':'sweet'}, but I don’t see another clearly ${savoryRequested?'savory':'sweet'} saved Nom that you haven’t already logged today. 🍡`,fresh);
...
function sipsAnswer(input,state,date){
 const summary=sipsSummary(state,date),drink=summary.drink||'Water',groups=drinkBreakdown(summary.entries),consumed=new Set(groups.map(group=>clean(group.name))),fridge=list(summary.fridge),fresh=fridge.filter(item=>!consumed.has(clean(item.name))).sort((a,b)=>Number(b.favorite)-Number(a.favorite)),fallback=fridge.slice().sort((a,b)=>Number(b.favorite)-Number(a.favorite)),sodaEntries=summary.entries.filter(entry=>isSodaName(entry.drink)),sodaOz=sodaEntries.reduce((sum,entry)=>sum+(Number(entry.amountOz)||0),0),wantsSweet=/\b(sweet|sugary|soda|juice|lemonade|sweet tea|something sweet|not water|no water)\b/.test(input),sweetFresh=fresh.filter(item=>isSweetDrinkName(item.name)),sweetFallback=fallback.filter(item=>isSweetDrinkName(item.name)),waterItem=fridge.find(item=>clean(item.name)==='water')||{name:'Water',servingOz:clean(drink)==='water'?summary.servingOz:0},needsWaterFirst=(sodaEntries.length>=2||sodaOz>=24)&&!wantsSweet;
 const breakdownText=groups.map(group=>`${group.name} ${group.total} oz`).join(' • '),currentQuestion=/\b(current drink|what am i sipping|what are we currently sipping)\b/.test(input),recommendQuestion=/\b(what should i drink|what should i sip|what do i drink|what are we sipping|pick.*drink|recommend.*drink|something to drink|something to sip|thirsty|sweet|sugary|not water|no water)\b/.test(input);
 ...
 if(recommendQuestion){
  const already=groups.length?`So far you’ve had ${breakdownText}. `:'';
  if(wantsSweet){const sweetPick=sweetFresh[0]||sweetFallback[0];if(sweetPick){const repeated=!sweetFresh.length&&consumed.has(clean(sweetPick.name));return response('cap_sips_recommendation',`${already}Sweet it is. ${repeated?'I’d circle back to':'I’d grab'} ${sweetPick.name}${sweetPick.servingOz?` (${sweetPick.servingOz} oz)`:''}. 🧊`,[sweetPick,...summary.entries],{signals:['sweet_requested',repeated?'repeat_sweet_option':'fresh_sweet_option']})}return response('cap_sips_recommendation',`${already}You asked for something sweet, but I don’t see a sweet drink saved in your Sip Fridge right now. 🧊`,summary.entries,{signals:['sweet_requested','no_saved_sweet_option']})}
  if(needsWaterFirst)return response('cap_sips_recommendation',`${already}Soda has already shown up at least twice today, so I’d make the next sip Water${waterItem.servingOz?` (${waterItem.servingOz} oz)`:''}. 💧`,[waterItem,...summary.entries],{signals:['water_first_after_soda']});
  const pick=fresh[0]||fallback[0];if(pick){const repeated=!fresh.length&&consumed.has(clean(pick.name));return response('cap_sips_recommendation',`${already}${repeated?'Your Sip Fridge doesn’t have an untried drink left today, so I’d circle back to':'I’d grab'} ${pick.name}${pick.servingOz?` (${pick.servingOz} oz)`:''}. 🧊`,[pick,...summary.entries],{signals:[repeated?'repeat_only_option':'fresh_fridge_option']})}
  return response('cap_sips_recommendation',`Your Sip Fridge is empty, so I only know your current quick-log drink: ${drink}. Add the drinks you have and I can make a real pick next time. 🧊`,[{drink}]);
 }
...
export function routeMochiniCapability(question,{state={},evaluation={},session={}}={}){
 ...
 const followupNoms=String(session?.lastIntent||'').startsWith('cap_noms_')&&/\b(anything|something|one|option|quicker|quick|else|another|savory|salty|sweet|dessert)\b/.test(input);
 const followupSips=String(session?.lastIntent||'').startsWith('cap_sips_')&&/\b(how much|enough|today|goal|what about now|else|another|option|sweet|sugary|soda|juice|water|not water|no water)\b/.test(input);
