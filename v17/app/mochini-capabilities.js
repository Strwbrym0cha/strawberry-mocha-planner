...
function remindersAnswer(input,state){
 let reminders=list(state.reminders).filter(item=>!item?.completed);if(/\bbirthday/.test(input))reminders=reminders.filter(item=>String(item?.type||'').toLowerCase()==='birthday'||String(item?.repeat||'').toLowerCase()==='yearly');
 reminders=reminders.slice().sort((a,b)=>`${a.date||'9999'} ${a.time||'99:99'}`.localeCompare(`${b.date||'9999'} ${b.time||'99:99'}`));if(!reminders.length)return response('cap_reminders',/\bbirthday/.test(input)?'I don’t see any upcoming birthday reminders saved right now. 🎂':'You do not have any open reminders right now. 🌷');
 return response('cap_reminders',`${/\bbirthday/.test(input)?'Upcoming birthday':'Next reminder'}: ${title(reminders[0])}${reminders[0].date?` • ${reminders[0].date}`:''}${reminders.length>1?` • ${reminders.length-1} more saved`:''}.`,reminders.slice(0,5));
}
...
export function routeMochiniCapability(question,{state={},evaluation={},session={}}={}){
 ...
 const selected=detected.ranked.slice(0,3),results=[];
 for(const {domain} of selected){let result=null;if(domain==='noms')result=nomsAnswer(input,state,date);else if(domain==='sips')result=sipsAnswer(input,state,date);else if(domain==='tasks')result=taskAnswer(input,state,date,evaluation);else if(domain==='routines')result=routineAnswer(input,state,date);else if(domain==='schedule')result=scheduleAnswer(input,state,baseDate);else if(domain==='deadlines')result=deadlinesAnswer(input,state,date);else if(domain==='capacity')result=capacityAnswer(evaluation);else if(domain==='hyperfixation')result=hyperfixationAnswer(state);else if(domain==='reminders')result=remindersAnswer(input,state);else if(domain==='goals')result=goalsAnswer(state);else if(domain==='wellness')result=wellnessAnswer(state,date);else if(domain==='projects')result=projectsAnswer(input,state);else if(domain==='katLabs')result=labsAnswer(input,state);else if(domain==='catchAll')result=catchAllAnswer(state);if(result){if(mentionsWork(input)&&(domain==='noms'||domain==='tasks'))result=appendWorkContext(result,state,date);results.push(result);}}
