...
import{eventMinutes,eventsForDate,nextTimedEvent}from'./events.js';
import{upcomingDeadlines}from'./deadlines.js';
import{routineSummaryForDate}from'./routines.js';
import{eligibleTasks,openTasksForDate,taskEligibility,taskTitle,tasksForDate}from'./tasks.js';
...
function recommendationFor(state,date,now){
 ...
 const scored=candidates.map(task=>({task,score:[deadlineRank(task,date),priorityRank(task.priority)]})).sort((a,b)=>b.score[0]-a.score[0]||b.score[1]-a.score[1]);
 const best=scored[0],tied=scored.filter(item=>item.score[0]===best.score[0]&&item.score[1]===best.score[1]);
 const describe=task=>{const eligibility=taskEligibility(state,task,date,{now}),reasons=[reason('assigned_today','Task is assigned to this planner day.'),reason('fits_capacity','Task passes the current capacity rule.')];if(task.dueDate&&deadlineRank(task,date)===3)reasons.unshift(reason('overdue','Task has a past due date.'));else if(task.dueDate&&deadlineRank(task,date)===2)reasons.unshift(reason('deadline_today','Task is due today.'));if(eligibility.nextFixedEvent)reasons.push(reason('before_fixed_event','Task fits before the next fixed event.'));return{task,reasons};};
...
export function evaluateToday(state={},context={}){
 const now=context.now instanceof Date?context.now:new Date(context.now||Date.now()),date=context.date||localDateKey(now),todayTasks=tasksForDate(state,date),openTasks=openTasksForDate(state,date),eligible=eligibleTasks(state,date,{now}),nextFixedEvent=nextTimedEvent(state,date,{now}),routines=routineSummaryForDate(state,date),deadlines=upcomingDeadlines(state,{date}),fixedMinutes=eventsForDate(state,date).reduce((sum,event)=>sum+eventMinutes(event),0),flexibleMinutes=todayTasks.filter(task=>!task.done&&!task.parked).reduce((sum,task)=>sum+(Number(task.durationMin)||0),0),availableMinutes=Math.max(0,16*60-fixedMinutes),alerts=[];
 ...
 const recommendation=recommendationFor(state,date,now),escalation={needsBigMochi:recommendation.ambiguous,reasons:recommendation.ambiguous?['multiple_competing_priorities']:[]};
