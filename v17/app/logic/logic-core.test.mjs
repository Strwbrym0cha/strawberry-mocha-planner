...
const gateway=evaluateToday({taskbot:{capacity:'High'},tasks:[{id:'shower',text:'Shower',date:day,routineId:'self-care',isGatewayTask:true},{id:'makeup',text:'Makeup',date:day,routineId:'self-care'}]},{date:day,now});assert.equal(gateway.recommendedNextAction.task.id,'shower');assert.ok(gateway.recommendedNextAction.reasons.some(reason=>reason.code==='gateway_task'));
const momentum=evaluateToday({taskbot:{capacity:'High'},tasks:[{id:'shower',text:'Shower',date:day,routineId:'self-care',done:true},{id:'skincare',text:'Skincare',date:day,routineId:'self-care'},{id:'desk',text:'Clean desk',date:day}]},{date:day,now});assert.equal(momentum.recommendedNextAction.task.id,'skincare');assert.ok(momentum.recommendedNextAction.reasons.some(reason=>reason.code==='routine_momentum'));
const deferred=evaluateToday({taskbot:{capacity:'High'},tasks:[{id:'deferred',text:'Shower',date:day,priority:'High',timesDeferred:3},{id:'other',text:'Other',date:day}]},{date:day,now});assert.ok(deferred.recommendedNextAction.reasons.some(reason=>reason.code==='deferred_repeatedly'));
const protectedFocus=evaluateToday({taskbot:{capacity:'High'},hyperfixation:{active:true,focusType:'project',focusId:'p1',focusLabel:'KatOS'},tasks:[{id:'focus',text:'Build KatOS',date:day,sourceProject:'p1'},{id:'work',text:'Required work videos',date:day,isProtected:true}]},{date:day,now});assert.equal(protectedFocus.recommendedNextAction.task.id,'work');assert.ok(protectedFocus.recommendedNextAction.reasons.some(reason=>reason.code==='protected_commitment'));assert.ok(protectedFocus.alerts.some(alert=>alert.code==='hyperfixation_protected_commitment'));
