...
state={...state,guidedRoutines:[{id:'night',name:'Night Routine',taskIds:['task-1'],gatewayTaskId:'task-1'}]};
assert.equal(actions.setHyperfixationExitRamp(store,{exitAt:'22:30',exitRoutineId:'night'}).ok,true,'an active session can save a user-selected exit ramp');
assert.equal(state.hyperfixation.exitAt,'22:30');assert.equal(state.hyperfixation.exitRoutineId,'night');
assert.equal((await import(data)).normalize(state).hyperfixation.exitRoutineId,'night','exit ramp survives persistence normalization');
