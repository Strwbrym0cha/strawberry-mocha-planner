...
const finance=url((await source('./logic/finance.js')).replace("'../data.js?v=22.1.16-20260817'",`'${data}'`));
const mochini=url((await source('./mochini.js')).replace("'./logic/finance.js?v=22.1.16-20260817'",`'${finance}'`));
...
const gatewayView=buildMochiniPresentation({...clear,recommendedNextAction:{task:{id:'gateway',text:'Shower',isGatewayTask:true},reasons:[{code:'gateway_task'}]}},state);assert.match(gatewayView.message,/first step/i);
const protectedView=buildMochiniPresentation({...clear,recommendedNextAction:{task:{id:'protected',text:'Work videos',isProtected:true},reasons:[{code:'protected_commitment'}]}},state);assert.match(protectedView.message,/protected commitment/i);
...
const requestWithConversation=buildBigMochiRequest(ambiguous,state,'Should I keep going?',{recentConversation:[{role:'kat',text:'I finished the first step.'},{role:'mochini',text:'The next step is ready.'}]});assert.match(requestWithConversation,/Relevant recent conversation/);assert.match(requestWithConversation,/Should I keep going/);assert.ok(!requestWithConversation.includes('income'));
...
assert.match(readableReason('gateway_task'),/gateway task/i);assert.match(readableReason('deferred_repeatedly'),/moved a few times/i);assert.match(readableReason('protected_commitment'),/protected commitment/i);
