...
export function createNomsActions(store){
 ...
 return{
  ...
  addGroceryItem(input={}){const name=clean(input.name);if(!name)return result(false,{error:'A grocery item needs a name.'});return updateNoms(noms=>{const now=stamp(),item={id:makeId('grocery'),name,quantity:clean(input.quantity),notes:clean(input.notes),obtained:false,...(clean(input.sourceCaptureId)?{sourceCaptureId:clean(input.sourceCaptureId)}:{}),createdAt:now,updatedAt:now};return{noms:{...noms,groceries:[...noms.groceries,item]},result:result(true,{item})}})},
