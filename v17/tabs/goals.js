...
function renderGoals({root,store}){
  ...
  const card=x=>{
    ...
    return `<article class="v17-goal-row"><div class="v17-goal-icon">${progress>=75?'🌙':'💭'}</div><div class="v17-goal-main"><div><b>${esc(x.title)}</b><span>${esc(x.category||'Personal')}</span></div><small>${esc(x.why||'One step at a time.')}</small><div class="v17-goal-progress"><i style="width:${progress}%"></i></div><small>${progress}%${x.date?' • target '+esc(x.date):''}</small></div><button class="sm-dream-progress" type="button" data-progress="${esc(x.id)}" style="--progress:${progress}%" aria-label="Increase ${esc(x.title)} progress from ${progress}%" title="Add 10% progress">${progress}%</button><button type="button" data-edit="${esc(x.id)}">✎</button><button type="button" data-archive="${esc(x.id)}">🗄</button></article>`;
  };
