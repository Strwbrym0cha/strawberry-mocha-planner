...
function render(root,store){
 const host=root.querySelector('.sm-noms'),fridge=root.querySelector('.sm-noms-fridge');if(!host||!fridge){root.querySelector('#sm-nom-diary')?.remove();return}
 const model=states.get(root)||{date:todayKey()};states.set(root,model);let panel=root.querySelector('#sm-nom-diary');if(!panel){panel=document.createElement('section');panel.id='sm-nom-diary';panel.className='sm-nom-diary';fridge.insertAdjacentElement('afterend',panel)}
 const state=store.get?.()||{},entries=history(state).filter(entry=>String(entry?.date||'')===String(model.date)).slice().sort((a,b)=>String(a.createdAt||'').localeCompare(String(b.createdAt||''))),rows=entries.map(entry=>({entry,label:foodLabel(state,entry),time:entry.createdAt?new Date(entry.createdAt).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}):''}));
 const sig=JSON.stringify([model.date,rows.map(row=>[row.entry.id,row.entry.createdAt,row.entry.nomId,row.entry.text,row.label])]);if(panel.dataset.sig===sig)return;panel.dataset.sig=sig;
 const dayLabel=model.date===todayKey()?'Today':new Date(`${model.date}T12:00:00`).toLocaleDateString([],{weekday:'long',month:'short',day:'numeric'});
 panel.innerHTML=`<div class="sm-nom-diary-head"><div><div class="v17-eyebrow">🍓 NOM DIARY</div><h2>What I ate ${model.date===todayKey()?'today':'that day'}</h2><small>Changing your current Nom adds a new entry instead of erasing the earlier ones.</small></div><div class="sm-nom-diary-date"><input type="date" data-nom-diary-date value="${esc(model.date)}"><button type="button" data-nom-diary-today>Today</button></div></div><div class="sm-nom-timeline">${rows.map(row=>`<article class="sm-nom-log"><time>${esc(row.time)}</time><span><b>🍱 ${esc(row.label)}</b><small>${esc(dayLabel)}</small></span><button type="button" data-nom-log-delete="${esc(row.entry.id)}" aria-label="Delete diary entry">×</button></article>`).join('')||'<div class="sm-nom-diary-empty">Nothing logged for this day yet. Your next Nom change will land here. 🍓</div>'}</div>`;
 panel.querySelector('[data-nom-diary-date]')?.addEventListener('change',event=>{model.date=event.target.value||todayKey();panel.dataset.sig='';render(root,store)});
 panel.querySelector('[data-nom-diary-today]')?.addEventListener('click',()=>{model.date=todayKey();panel.dataset.sig='';render(root,store)});
